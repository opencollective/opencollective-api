    ...
    if (includeGiftCards) {
      where = {
        ...
        [Op.or]: {
          ...
          UsingGiftCardFromCollectiveId: collectiveIds,
        },
      };
    } else {
      where.CollectiveId = collectiveIds;
    }
    if (excludeCrossCollectiveTransactions) {
      where.FromCollectiveId = { [Op.notIn]: collectiveIds };
    }
  }
  if (transactionType) {
...
export async function sumCollectivesTransactions(collectiveIds, options) {
  const { column, startDate, endDate, includeGiftCards } = options;
  const where = computeCollectiveTransactionsWhere(collectiveIds, options);
  ...
  if (options.withBlockedFunds) {
    ...
    for (const blockedFundResult of blockedFundResults) {
      ...
      totals[CollectiveId].value -= Math.round(blockedFundResult['amount'] * fxRate);
    }
  }
