...
function getAmountColumn({ column }) {
  switch (column) {
    case 'amountInCollectiveCurrency':
      return [sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('amount')), 0), 'amountInCollectiveCurrency'];
    case 'netAmountInCollectiveCurrency':
      return [
        sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('netAmountInCollectiveCurrency')), 0),
        'netAmountInCollectiveCurrency',
      ];

    case 'amountInHostCurrency':
      return [
        sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('amountInHostCurrency')), 0),
        'amountInHostCurrency',
      ];

    case 'netAmountInHostCurrency':
      return [
        sequelize.fn(
          'COALESCE',
          sequelize.literal(
            ...
            + SUM(COALESCE("taxAmount" * "hostCurrencyFxRate", 0))`,
          ),
          0,
        ),
...
function getCurrencyColumn({ column }) {
  return ['amountInHostCurrency', 'netAmountInHostCurrency'].includes(column) ? 'hostCurrency' : 'currency';
}
...
export async function getBalanceWithBlockedFundsAmount(collective, { currency, version, loaders } = {}) {
  ...
  const results = sumCollectivesTransactions([collective.id], {
    currency: currency,
    column: ['v0', 'v1'].includes(version) ? 'netAmountInCollectiveCurrency' : 'netAmountInHostCurrency',
    excludeRefunds: false,
...
export function computeCollectiveTransactionsWhere(
  ids,
  {
    startDate = null,
    endDate = null,
...
export async function sumCollectivesTransactions(ids, options) {
  const { column, startDate, endDate, includeGiftCards, withBlockedFunds } = options;
  const where = computeCollectiveTransactionsWhere(ids, options);
  ...
  const amountColumn = getAmountColumn(options);
  const currencyColumn = getCurrencyColumn(options);
  const collectiveIdColumn = includeGiftCards
    ? [
  ...
  const results = await models.Transaction.findAll({
    attributes: [collectiveIdColumn, amountColumn, currencyColumn],
    where,
    group: [collectiveIdColumn, currencyColumn],
    raw: true,
  });
  ...
  for (const result of results) {
    ...
    if (!totals[CollectiveId]) {
      totals[CollectiveId] = { CollectiveId, value: 0, currency: null };
    }
    // If it's the first total collected, set the currency
    if (!totals[CollectiveId].currency) {
    ...
    totals[CollectiveId].value += Math.round(result[column] * fxRate);
  }
  ...
  if (withBlockedFunds) {
    ...
    const disputedTransactions = await models.Transaction.findAll({
      attributes: ['CollectiveId', amountColumn, currencyColumn],
...
export async function groupCollectivesTransactions(ids, options) {
  const { column, includeGiftCards } = options;
  const where = computeCollectiveTransactionsWhere(ids, options);
  ...
  const results = await models.Transaction.findAll({
    attributes: [collectiveIdColumn, 'FromCollectiveId', amountColumn, currencyColumn],
    where,
  ...
  for (const result of results) {
    ...
    const groupBy = `${CollectiveId}-${FromCollectiveId}`;
    ...
    totals[groupBy].value += Math.round(result[column] * fxRate);
  }
