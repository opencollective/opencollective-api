...
export async function associateTransactionRefundId(transaction, refund, data) {
  ...
  if (data) {
    refundCredit.data = data;
    refundDebit.data = data;
  }

  refundCredit.RefundTransactionId = credit.id;
  await refundCredit.save(); // User Ledger
  refundDebit.RefundTransactionId = debit.id;
  await refundDebit.save(); // Collective Ledger
  debit.RefundTransactionId = refundDebit.id;
  await debit.save(); // Collective Ledger
  credit.RefundTransactionId = refundCredit.id;
  await credit.save(); // User Ledger
