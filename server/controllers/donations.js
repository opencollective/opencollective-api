...
module.exports = (app) => {
  ...
  const stripeDonation = (req, res, next) => {
    ...
    // create a new subscription (this needs to happen first, because of hook on Donation model)
    .then(() => {
      if (isSubscription) {
    ...
    // TODO: move this over to processDonation. Need to refactor emailLib first.
    .then(() => emailLib.send(
      'thankyou',
      req.user.email,
      {
