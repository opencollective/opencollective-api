...
import Transaction, { TransactionModelInterface } from './Transaction';
...
> {
  ...
  static async markTransactionsAsInvoiced(transactions: TransactionModelInterface[], expenseId: number): Promise<void> {
  ...
  static async getByTransaction(transaction: TransactionModelInterface): Promise<TransactionSettlement> {
    return TransactionSettlement.findOne({
      where: { TransactionGroup: transaction.TransactionGroup, kind: transaction.kind },
    });
  }

  static async attachStatusesToTransactions(transactions: TransactionModelInterface[]): Promise<void> {
