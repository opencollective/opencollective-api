...
export default function(Sequelize, DataTypes) {
      ...
      hooks: {
        beforeCreate: instance => {
          if (instance.service !== 'opencollective') {
            if (!instance.token) {
              throw new Error(
                `${instance.service} payment method requires a token`,
              );
            }
            if (
              instance.service === 'stripe' &&
              !instance.token.match(/^(tok|src)_[a-zA-Z0-9]{24}/)
            ) {
              if (
  ...
  PaymentMethod.prototype.canBeUsedForOrder = async function(order, user) {
    ...
    const name = this.matching
      ? 'matching fund'
      : `payment method (${this.service}:${this.type})`;
    ...
    if (!this.CollectiveId) {
      ...
      if (order.matchingFund !== this.uuid.substr(0, 8)) {
        throw new Error(
          'This payment method can only be used to match an order',
        );
      }
    } else {
    ...
    if (orderCurrency !== this.currency) {
      orderAmountInfo += ` ~= ${formatCurrency(
        totalAmountInPaymentMethodCurrency,
        this.currency,
      )}`;
    }
    if (
      this.monthlyLimitPerMember &&
      totalAmountInPaymentMethodCurrency > this.monthlyLimitPerMember
    ) {
      throw new Error(
  ...
  PaymentMethod.prototype.getBalanceForUser = async function(user) {
    if (user && !(user instanceof models.User)) {
      throw new Error(
        'Internal error at PaymentMethod.getBalanceForUser(user): user is not an instance of User',
      );
    }

    const paymentProvider = libpayments.findPaymentMethodProvider(this);
    const getBalance =
      paymentProvider && paymentProvider.getBalance
    ...
    if (
      !this.initialBalance &&
      (!this.monthlyLimitPerMember || (user && user.isAdmin(this.CollectiveId)))
    ) {
    ...
    const result = await sumTransactions(
  ...
  PaymentMethod.createFromStripeSourceToken = PaymentMethodData => {
    ...
    return stripe
      .createCustomer(null, PaymentMethodData.token)
      .then(customer => {
  ...
  PaymentMethod.getOrCreate = (user, paymentMethod) => {
    if (!paymentMethod.uuid) {
      ...
      paymentMethod.uuid &&
      libpayments.isProvider('opencollective.giftcard', paymentMethod)
    ) {
  ...
  PaymentMethod.getMatchingFund = (shortUUID, options = {}) => {
    ...
    return PaymentMethod.findOne({
      ...
      if (pm.limitedToCollectiveIds) {
        if (
          !options.ForCollectiveId ||
          pm.limitedToCollectiveIds.indexOf(options.ForCollectiveId) === -1
        ) {
          throw new Error(
            'This matching fund is not available for this collective',
          );
        }
      }
      if (pm.limitedToTags) {
        ...
        if (intersection(collective.tags, pm.limitedToTags).length === 0) {
          throw new Error(
            'This matching fund is not available to collectives in this category',
          );
        }
      }
      return pm;
