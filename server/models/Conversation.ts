...
class Conversation extends Model<Conversation> {
  ...
  static createWithComment = async function(user, collective, title, html, tags = null): Promise<Conversation> {
  ...
  static getMostPopularTagsForCollective = async function(
    collectiveId,
    limit = 100,
  ): Promise<{ id: string; tag: string; count: number }[]> {
   ...
   */
  getUsersFollowing = async function(): Promise<any[]> {
...
export default function(sequelize, DataTypes): typeof Conversation {
  Conversation.init(
    {
      id: {
