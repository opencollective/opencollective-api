...
import type { Collective, Order } from '.';
...
> {
  ...
  async canBeDeleted(): Promise<boolean> {
    const orderCount = await (sequelize.models.Order as typeof Order).count({
   ...
   */
  async archive(): Promise<ManualPaymentProvider> {
    return this.update({ archivedAt: new Date() });
  }
}
