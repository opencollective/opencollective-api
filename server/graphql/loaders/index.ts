        ...
        return Promise.all(
          keys.map(async key => {
            ...
            for (const [resultCurrency, resultAmount] of Object.entries(results)) {
              ...
              if (isNil(fxRate)) {
                // Edge-case: we're not able to get the FX rate. Log & skip.
                reportMessageToSentry('Unable to get FX rate for total amount donated', {
                  req,
                  extra: {
                    fromCollectiveId: key.FromCollectiveId,
                    collectiveId: key.CollectiveId,
                    resultCurrency,
                    resultAmount,
            ...
            return roundCentsAmount(total, key.currency);
          }),
        );
      }),
