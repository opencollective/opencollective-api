...
import models, { Member, Op } from '../../../../models';
...
const MembersCollectionQuery = {
  ...
  async resolve(_: void, args, req: Express.Request) {
    ...
    const where: WhereOptions<InferAttributes<Member>> & {
      [Op.and]: WhereOptions<InferAttributes<Member>>[];
    } = { [Op.and]: [] };
    ...
    if (args.account) {
      ...
      if (!isUndefined(args.tier)) {
        if (args.tier) {
          ...
          where[Op.and].push({ TierId: tier.id });
        } else {
          where[Op.and].push({ TierId: { [Op.is]: null } });
        }
      }
    } else if (args.host) {
