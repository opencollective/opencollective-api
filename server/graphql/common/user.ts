...
export const createUser = (
  ...
  const runInTransaction = async transaction => {
    ...
    if (organizationData) {
      ...
      // If the autogenerated user slug conflicts with the organization slug, we update the user slug to avoid conflicts
      if (organizationParams.slug && organizationParams.slug === user.collective.slug) {
        await user.collective.update(
          { slug: `${user.collective.slug}-${cryptoRandomString({ length: 8 })}` },
          { transaction },
        );
      }
