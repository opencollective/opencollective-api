...
/**
 * Settlements and platform billing expenses are generated by the platform and billed to the host. The host admins
 * are also the admins of the expense's collective, so they must not get the regular "collective/host admin" powers on them.
 */
export const isPlatformExpense = (expense: Expense): boolean =>
...
export const canEditExpense: ExpensePermissionEvaluator = async (
  ...
  } else if (isPlatformExpense(expense) && !nonEditableStatuses.includes(expense.status)) {
...
export const canMarkAsPaid: ExpensePermissionEvaluator = async (
  ...
  if (!validateExpenseScope(req, options)) {
    ...
    const host = await loadHostForExpense(req, expense);
    if (isBlockedForUnpaidPlatformBilling(host) && !isPlatformExpense(expense)) {
      if (options?.throw) {
...
export const canApprove: ExpensePermissionEvaluator = async (
  ...
  } else if (expense.type === ExpenseType.PLATFORM_BILLING) {
    // Platform bills are created approved; they only go back to pending when a platform admin edits them.
    // Settlements are created pending and approved by the billed host, so they keep the regular flow.
...
export const canMarkAsIncomplete: ExpensePermissionEvaluator = async (
  ...
  if (!validateExpenseScope(req, options)) {
    ...
    return false;
  } else if (isPlatformExpense(expense)) {
    if (options?.throw) {
      throw new Forbidden(
...
export const checkHasBalanceToPayExpense = async (
  ...
  if (expense.feesPayer === 'PAYEE') {
    assert(!isPlatformExpense(expense), 'The payee cannot cover the fees on platform settlements');
    assert(
  ...
  if (forceManual) {
    ...
    if (!isPlatformExpense(expense) && balanceInHostCurrency < totalAmountPaidInHostCurrency) {
      throw new Error(
