...
async function connectStripeAccount({
  ...
  // Exchange the OAuth code and retrieve the Stripe account details before touching the DB,
  // so that a Stripe API failure does not destroy the existing connected account.
  ...
  // Replace any existing Stripe connected account within a transaction so the destroy and
  // create are atomic — either both succeed or neither is persisted.
