...
import { groupBy, keyBy, padStart, pick, sumBy } from 'lodash';
...
import { exportToPDF, parseToBoolean, sumByWhen } from '../server/lib/utils';
...
const GENERATE_LEGACY_REPORT = parseToBoolean(process.env.GENERATE_LEGACY_REPORT);
...
const sendEmail = (emailTemplate, recipients, data, attachments) => {
...
async function HostReport(year, month, hostId) {
  ...
  const processHost = async host => {
    try {
      ...
      data.reportSlug = yearlyReport ? `${year}` : `${year}-${padStart(month + 1, 2, '0')}`;
