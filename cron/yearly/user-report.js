...
const buildTweet = (fromCollective, collectives, totalDonations) => {
  ...
  for (const slug in collectives) {
    ...
    collectivesDonationsNames.push(`${formatCurrency(collective.totalDonations, collective.currency)} across ${collectiveName}`);
  }
  const pronoun = fromCollective.type === 'USER' ? 'I' : 'we';
  if (totalCollectives > 2) {
    tweet = `🎁 In ${year}, ${pronoun} have contributed a total of ${formatCurrencyObject(totalDonations)} across ${totalCollectives} collectives`;
    ...
    tweet = `🎁 In ${year}, ${pronoun} have contributed ${collectivesDonationsNames[0].replace(' to ',' to the ')} collective`;
  } else {
...
const processCollective = (collective) => {
  ...
  .then(transactions => {
    ...
    return {
      stats: { totalCollectives, totalDonations, totalDonationsString: formatCurrencyObject(totalDonations) },
