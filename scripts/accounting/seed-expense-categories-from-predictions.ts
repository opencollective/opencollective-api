...
type Prediction = { code: string; name: string; confidence: number };
type ExpensePrediction = { id: string; predictions: Prediction[] };

type FetchPredictionsResult = { expenses: ExpensePrediction[] };
...
async function fetchPredictionsWithEmbedding(
  hostSlug: string,
  mlInputs: { id: string; description: string; items: string; appliesTo: string }[],
): Promise<FetchPredictionsResult> {
  const url = `${ML_API_URL}/models/expense-category/embeddings/predict`;
...
async function fetchPredictionsWithSVC(
  hostSlug: string,
  mlInputs: { id: string; description: string; items: string; appliesTo: string }[],
): Promise<FetchPredictionsResult> {
  // SVC endpoint expects one expense at a time
  const results: ExpensePrediction[] = [];
  for (const input of mlInputs) {
    ...
    const url = `${ML_API_URL}/models/expense-category?${params.toString()}`;
...
async function fetchPredictionsWithLLM(
  hostSlug: string,
  mlInputs: { id: string; description: string; items: string; appliesTo: string }[],
  categories: AccountingCategory[],
): Promise<FetchPredictionsResult> {
  const url = `${ML_API_URL}/models/expense-category/llm/predict`;
  const response = await axios.post(
    url,
    {
      /* eslint-disable camelcase */
...
const writeCsvLine = (expense: Expense, category: AccountingCategory, csvStream: fs.WriteStream) => {
  ...
  csvStream.write(`${expenseUrl},${category.code},${category.name}\n`);
};
...
const run = async (hostSlug: string, model: string, options: RunOptions) => {
  ...
  if (options.csv) {
    ...
    csvStream.write('EXPENSE_URL,CATEGORY_CODE,CATEGORY_NAME\n');
  }
  ...
  for (let i = 0; i < expenses.length; i += BATCH_SIZE) {
    ...
    let mlExpenses: ExpensePrediction[] = [];
    try {
      let result: FetchPredictionsResult;
      if (model === 'SVC') {
        result = await fetchPredictionsWithSVC(hostSlug, mlInputs);
      } else if (model === 'EMBEDDING') {
        result = await fetchPredictionsWithEmbedding(hostSlug, mlInputs);
      } else {
        result = await fetchPredictionsWithLLM(hostSlug, mlInputs, categories);
      }
      mlExpenses = result.expenses || [];
    } catch (error) {
      logger.error('ML API call failed:', error?.response?.data || error.message);
      continue;
    }
