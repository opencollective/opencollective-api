...
app.get('/preview/:template', async (req, res) => {
    ...
    res.status(404).send(`
      <!DOCTYPE html>
        ...
        <body>
          ...
          <p>Template not found: <code>${templateName}</code></p>
        </body>
      </html>
    `);
    return;
  ...
  res.send(`
    ...
    <body>
      <a href="/">&#x2190; Back to templates</a>
      <h1>Rendering template: <code>${templateName}</code></h1>
      <div class="subject-container">
        <span>Subject:</span>
        <iframe src="/render/${templateName}/title"></iframe>
      </div>
      <iframe class="content-preview" src="/render/${templateName}"></iframe>
      <h2>Text version</h2>
      <pre class="text-preview">${EmailLib.generateEmailFromTemplate(templateName, 'test@opencollective.com', mockData).text}</pre>
    </body>
    </html>
  `);
});
...
app.get('/render/:template', async (req, res) => {
  ...
  try {
    ...
    const renderResult = EmailLib.generateEmailFromTemplate(templateName, 'test@opencollective.com', mockData);
