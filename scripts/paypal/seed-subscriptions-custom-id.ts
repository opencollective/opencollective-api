...
import { paypalRequest } from '../../server/paymentProviders/paypal/api';
...
const main = async (): Promise<void> => {
  ...
  for (const order of orders) {
    ...
    const customId = `Contribution #${order.id}`.slice(0, customIdMaxLength);
    ...
    try {
      ...
      if (dryRun) {
        logger.info(
          `[dry-run] Would set custom_id="${customId}" on PayPal subscription ${paypalSubscriptionId} (Order #${order.id})`,
        );
        updated++;
        continue;
      }
      ...
      await paypalRequest(
        ...
        [{ op: 'replace', path: '/custom_id', value: customId }],
        host,
        'PATCH',
        { shouldReportErrors: false },
      );
      updated++;
      logger.info(`Set custom_id="${customId}" on subscription ${paypalSubscriptionId} (Order #${order.id})`);
    } catch (e) {
      failed++;
