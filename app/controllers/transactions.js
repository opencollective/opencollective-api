    ...
    async.auto({
      ...
      checkPaymentMethod: ['getPaymentMethod', function(cb, results) {
        if (isManual) {
          ...
          return cb(new errors.BadRequest('This user has no confirmed payment method linked with this service.'));
        } else {
          return cb(null, results.getPaymentMethod.rows[0]); // Use first payment method.
        }
      }],
