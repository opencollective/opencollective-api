...
/**
 * Extends `HostedCollectivesDailyFinancialActivity` with:
 *  - per-kind transaction counts: `contributionsCount` (CREDIT) and `payoutsCount` (DEBIT, ex-fees),
 *    using the exact filters behind `amountReceived` / `amountSpent` so avg size stays coherent;
 *  - a `contributionFrequency` dimension (ONE_TIME / RECURRING / ADDED_FUNDS; OTHER for payouts and
 *    non-contribution credits), keyed off the order interval.
 *
 * The frequency CASE is part of the GROUP BY, so each (host, collective, day, frequency) is a single
 * row — totals for queries that don't group by frequency are unchanged. `down` restores the
 * pre-existing (counts/frequency-free) view from `20260424120000`.
 */
  ...
  async up(queryInterface) {
    ...
    await queryInterface.sequelize.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS "hosted_collectives_daily_financial_activity__unique_index"
        ON "HostedCollectivesDailyFinancialActivity" ("HostCollectiveId", "CollectiveId", "day", "contributionFrequency");
      CREATE INDEX IF NOT EXISTS "hosted_collectives_daily_financial_activity__host_day"
        ...
        INCLUDE ("CollectiveId", "ParentCollectiveId", "collectiveType", "mainAccountType", "isArchived", "mainAccountIsArchived", "contributionFrequency", "amountReceived", "amountReceivedNet", "amountSpent", "amountSpentNet");
      CREATE INDEX IF NOT EXISTS "hosted_collectives_daily_financial_activity__host_collective"
        ...
        INCLUDE ("day", "collectiveType", "mainAccountType", "isArchived", "mainAccountIsArchived", "contributionFrequency", "amountReceived", "amountReceivedNet", "amountSpent", "amountSpentNet");
    `);
  },

  async down(queryInterface) {
    // Restore the pre-existing view (no counts / no frequency) from 20260424120000.
    await queryInterface.sequelize.query(`DROP MATERIALIZED VIEW IF EXISTS "HostedCollectivesDailyFinancialActivity";`);
    await queryInterface.sequelize.query(`
      CREATE MATERIALIZED VIEW "HostedCollectivesDailyFinancialActivity" AS (
        SELECT
