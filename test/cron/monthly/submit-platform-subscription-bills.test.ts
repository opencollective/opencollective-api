...
describe('submit-platform-subscription-bills', () => {
  ...
  it('should omit bills for $0', async () => {
    const expenses = await models.Expense.findAll({
      where: { CollectiveId: organizations[0].id, type: expenseTypes.SETTLEMENT },
    });
  ...
  it('should submit Expenses for active subscriptions', async () => {
    const expenses = await models.Expense.findAll({
      where: { type: expenseTypes.SETTLEMENT },
      order: [['id', 'DESC']],
    });
  ...
  it('should not bill organizations twice', async () => {
    ...
    const expenses = await models.Expense.findAll({
      where: { CollectiveId: organizations[1].id, type: expenseTypes.SETTLEMENT },
    });
