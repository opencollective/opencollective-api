...
export function stubStripeBalance(sandbox, amount, currency, applicationFee = 0, stripeFee = 0) {
  ...
  sandbox.stub(stripeGateway, 'retrieveBalanceTransaction').callsFake(async () => balanceTransaction);
  sandbox.stub(stripe.balanceTransactions, 'retrieve').callsFake(async () => balanceTransaction);
}
