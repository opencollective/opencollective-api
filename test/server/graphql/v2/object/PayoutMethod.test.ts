...
describe('server/graphql/v2/object/PayoutMethod', () => {
  ...
  describe('canBeDeleted', () => {
    it('returns true for payment methods that can be deleted , false if only archived archived', async () => {
      ...
      const pm = await fakePayoutMethod({
        ...
        name: 'cannotBeDeleted - is used in expenses',
        CollectiveId: user.CollectiveId,
      });
      await fakeExpense({
