...
describe('TransactionsCollectionQuery - includePlatformTransactions', () => {
  let hostAdminUser, host, collective, platformTipsAccount;
  ...
  const query = gql`
    query Transactions(
      ...
      $includePlatformTransactions: Boolean
      ...
      transactions(
        ...
        includePlatformTransactions: $includePlatformTransactions
  ...
  before(async () => {
    ...
    host = await fakeActiveHost({ admin: hostAdminUser.collective });
    collective = await fakeCollective({ HostCollectiveId: host.id });
    // The global, host-less PLATFORM account (single source of truth: PlatformConstants.PlatformOwnedAccountSlugs)
    ...
    // A regular hosted-collective contribution (host-scoped, on the collective).
    await fakeTransaction({
      CollectiveId: collective.id,
      HostCollectiveId: host.id,
      kind: TransactionKind.CONTRIBUTION,
    ...
    // A platform tip held on the host-scoped slice of the platform-tips account.
    await fakeTransaction({
  ...
  it('Fiscal Host view: account=host + includePlatformTransactions includes the platform-owned account', async () => {
    const result = await graphqlQueryV2(
      query,
      {
        host: { slug: host.slug },
  ...
  it('Hosted Collectives view: excludeAccount=host + includePlatformTransactions drops the platform-owned account', async () => {
    const result = await graphqlQueryV2(
      query,
      {
        host: { slug: host.slug },
        excludeAccount: [{ slug: host.slug }],
        includePlatformTransactions: true,
        includeChildrenTransactions: true,
      },
      hostAdminUser,
    ...
    expect(slugs).to.include(collective.slug);
  });

  it('without the flag, account=host does not pull in the platform-owned account', async () => {
    const result = await graphqlQueryV2(
      query,
      {
        host: { slug: host.slug },
