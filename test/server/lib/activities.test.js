  ...
  describe('formatMessageForPublicChannel', () => {
    it(`${constants.COLLECTIVE_TRANSACTION_CREATED} donation`, () => {
      ...
      expect(message).to.equal(
        'New Donation: someone gave USD 10.42 to <https://opencollective.com/pubquiz|Pub quiz>!',
      );
    });
    ...
    it(`${constants.COLLECTIVE_EXPENSE_PAID} expense paid`, () => {
      ...
      expect(message).to.equal("Expense paid on <https://opencollective.com/pubquiz|Pub quiz>: USD -12.98 for 'pizza'");
    });

    it(constants.SUBSCRIPTION_CONFIRMED, () => {
      const { message } = activitiesLib.formatMessageForPublicChannel(activitiesData[14], 'slack');
      expect(message).to.equal(
    ...
    it(constants.COLLECTIVE_CREATED, () => {
      const { message } = activitiesLib.formatMessageForPublicChannel(activitiesData[17], 'slack');
    ...
    it(`${constants.COLLECTIVE_EXPENSE_CREATED}`, () => {
      ...
      expect(message).to.equal(
        'New Expense: someone submitted an expense to <blah.com|Blah>: EUR 12.34 for for pizza!',
      );
    });
    ...
    it(`${constants.COLLECTIVE_EXPENSE_REJECTED}`, () => {
      ...
      expect(message).to.equal('Expense rejected: EUR 12.34 for for pizza in <blah.com|Blah>!');
    });
    ...
    it(`${constants.COLLECTIVE_EXPENSE_APPROVED}`, () => {
      ...
      expect(message).to.equal('Expense approved: EUR 12.34 for for pizza in <blah.com|Blah>!');
    });
  });

  describe('formatMessageForPublicChannel gitter format', () => {
    it(`${constants.SUBSCRIPTION_CONFIRMED} with month interval for Gitter`, () => {
      const { message } = activitiesLib.formatMessageForPublicChannel(activitiesData[15], 'markdown');
      expect(message).to.equal(
