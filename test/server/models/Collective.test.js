...
describe('server/models/Collective', () => {
  ...
  describe('settings', () => {
    ...
    describe('customPaymentProviders', () => {
      const validBankTransferProvider = {
        id: '01945f18-b7e4-7000-8000-000000000001',
        type: 'BANK_TRANSFER',
        name: 'Wire Transfer',
      ...
      const validOtherProvider = {
        id: '01945f18-b7e4-7000-8000-000000000002',
        type: 'OTHER',
        name: 'Custom Payment',
      ...
      describe('validations', () => {
        it('accepts valid BANK_TRANSFER provider', async () => {
          const collective = await fakeCollective({
            settings: { customPaymentProviders: [validBankTransferProvider] },
          });
          expect(collective.settings.customPaymentProviders).to.deep.equal([validBankTransferProvider]);
        });

        it('accepts valid OTHER provider', async () => {
          const collective = await fakeCollective({
            settings: { customPaymentProviders: [validOtherProvider] },
          });
          expect(collective.settings.customPaymentProviders).to.deep.equal([validOtherProvider]);
        });

        it('accepts multiple providers', async () => {
          const collective = await fakeCollective({
            settings: { customPaymentProviders: [validBankTransferProvider, validOtherProvider] },
          });
          expect(collective.settings.customPaymentProviders).to.have.length(2);
        });

        it('rejects provider with missing id', async () => {
          const providerWithoutId = { ...validBankTransferProvider };
          delete providerWithoutId.id;
          await expect(
            fakeCollective({ settings: { customPaymentProviders: [providerWithoutId] } }),
          ).to.be.rejectedWith('Validation error');
        });

        it('rejects provider with invalid UUID format', async () => {
          await expect(
            fakeCollective({
              settings: {
                customPaymentProviders: [{ ...validBankTransferProvider, id: 'invalid-uuid' }],
              },
            }),
          ).to.be.rejectedWith('Validation error');
        });
        ...
        it('rejects provider with missing name', async () => {
          const providerWithoutName = { ...validBankTransferProvider };
          delete providerWithoutName.name;
          await expect(
            fakeCollective({ settings: { customPaymentProviders: [providerWithoutName] } }),
          ).to.be.rejectedWith('Validation error');
        });

        it('rejects provider with missing instructions', async () => {
          const providerWithoutInstructions = { ...validBankTransferProvider };
          delete providerWithoutInstructions.instructions;
          await expect(
            fakeCollective({ settings: { customPaymentProviders: [providerWithoutInstructions] } }),
          ).to.be.rejectedWith('Validation error');
        });

        it('rejects BANK_TRANSFER provider with missing accountDetails', async () => {
          const providerWithoutAccountDetails = { ...validBankTransferProvider };
          delete providerWithoutAccountDetails.accountDetails;
          await expect(
            fakeCollective({ settings: { customPaymentProviders: [providerWithoutAccountDetails] } }),
          ).to.be.rejectedWith('Validation error');
        });
        ...
        it('rejects provider with invalid type', async () => {
          await expect(
            fakeCollective({
              settings: {
                customPaymentProviders: [{ ...validBankTransferProvider, type: 'INVALID_TYPE' }],
              },
            }),
          ).to.be.rejectedWith('Validation error');
        });
            ...
            fakeCollective({
              settings: {
                customPaymentProviders: [
                  validBankTransferProvider,
                  { ...validOtherProvider, id: validBankTransferProvider.id },
                ],
              },
            }),
          ).to.be.rejectedWith('Custom payment providers IDs must be unique');
        });
        ...
        it('accepts empty array', async () => {
          ...
          expect(collective.settings.customPaymentProviders).to.deep.equal([]);
        });

        it('accepts optional icon field', async () => {
          const providerWithIcon = { ...validBankTransferProvider, icon: 'bank' };
          const collective = await fakeCollective({
            settings: { customPaymentProviders: [providerWithIcon] },
          });
          expect(collective.settings.customPaymentProviders[0].icon).to.equal('bank');
        });
      });
      ...
      describe('sanitization', () => {
        it('removes dangerous HTML tags like <script>', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.equal('<p>Pay here</p>');
        });
        ...
        it('keeps safe HTML tags like <strong>, <em>, <a>', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<strong>Important:</strong>');
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<em>exact</em>');
          expect(collective.settings.customPaymentProviders[0].instructions).to.include(
            'href="https://github.com/opencollective"',
          );
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<a ');
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('this link</a>');
        });
        ...
        it('removes iframe tags', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.equal('<p>Instructions</p>');
          expect(collective.settings.customPaymentProviders[0].instructions).to.not.include('iframe');
        });
        ...
        it('removes onclick and other event handlers', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.not.include('onclick');
        });
        ...
        it('preserves formatting tags like <ul>, <ol>, <li>', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<ul>');
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<li>');
        });
        ...
        it('preserves images', async () => {
          ...
          expect(collective.settings.customPaymentProviders[0].instructions).to.include('<img');
          expect(collective.settings.customPaymentProviders[0].instructions).to.include(
            'src="https://example.com/qr.png"',
          );
        });

        it('sanitizes instructions for multiple providers', async () => {
          const collective = await fakeCollective({
            settings: {
              customPaymentProviders: [
                {
                  ...validBankTransferProvider,
                  instructions: '<p>Safe</p><script>bad</script>',
                },
                {
                  ...validOtherProvider,
                  instructions: '<div>OK</div><script>evil</script>',
                },
              ],
            },
          });
          expect(collective.settings.customPaymentProviders[0].instructions).to.equal('<p>Safe</p>');
          expect(collective.settings.customPaymentProviders[1].instructions).to.equal('<div>OK</div>');
        });
      });
    });
  });
